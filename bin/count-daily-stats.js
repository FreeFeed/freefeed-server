...
async function get_next_metric_update_date(metric) {
  ...
  const res = await postgres('stats')
    .max('dt')
    .where('metric', metric);

  if (res[0].max) {
    return moment(res[0].max)
      .add(1, 'd')
      .startOf('day');
  }
...
async function main() {
  ...
  await create_metric('posts', to_date, async (dt, next_date) => {
    const res = await postgres('posts')
      .count('id')
      .where('created_at', '<', next_date)
      .first();
    return res.count;
  ...
  await create_metric('comments', to_date, async (dt, next_date) => {
    const res = await postgres('comments')
      .count('id')
      .where('created_at', '<', next_date)
      .first();
    return res.count;
  ...
  await create_metric('likes', to_date, async (dt, next_date) => {
    const res = await postgres('likes')
      .count('id')
      .where('created_at', '<', next_date)
      .first();
    return res.count;
  ...
  await create_metric('events', to_date, async (dt, next_date) => {
    const res = await postgres('events')
      .count('id')
      .where('created_at', '<', next_date)
      .first();
    return res.count;
