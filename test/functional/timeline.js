...
describe('TimelinesController', () => {
  ...
  describe('#posts()', () => {
    ...
    it('should respond with 404 for "deleted" user', async () => {
      ...
      return expect(
        funcTestHelper.getUserFeed(context),
        'to be rejected with',
        new Error('HTTP/1.1 404'),
      );
    });
  });
  ...
  describe('#pagination', () => {
    ...
    it('should respect explicit pagination limits', (done) => {
      funcTestHelper.getTimelinePaged(
        `/v2/timelines/${context.username}`,
        context.authToken,
        0,
        1,
  ...
  describe('#likes()', () => {
    ...
    it('should return empty likes timeline after un-like', (done) => {
      request
        ...
        .end(() => {
          funcTestHelper.getTimeline(
            `/v2/timelines/${context.username}/likes`,
            context.authToken,
            (err, res) => {
              res.should.not.be.empty;
              res.body.should.not.be.empty;
    ...
    it('should respond with 404 for "deleted" user', async () => {
      await dbAdapter.updateUser(context.user.id, { hashedPassword: '' });
      return expect(
        funcTestHelper.getUserLikesFeed(context),
        'to be rejected with',
        new Error('HTTP/1.1 404'),
      );
    });
  ...
  describe('#comments()', () => {
    ...
    it('should return comments timeline', (done) => {
      funcTestHelper.getTimeline(
        `/v2/timelines/${context.username}/comments`,
        context.authToken,
        (err, res) => {
          res.should.not.be.empty;
          res.body.should.not.be.empty;
    ...
    it('should respond with 404 for "deleted" user', async () => {
      await dbAdapter.updateUser(context.user.id, { hashedPassword: '' });
      return expect(
        funcTestHelper.getUserCommentsFeed(context),
        'to be rejected with',
        new Error('HTTP/1.1 404'),
      );
    });
