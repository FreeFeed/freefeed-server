...
describe('All groups', () => {
  ...
  describe('There are three groups: Pubic, Protected and Private. Luna, Mars and Venus wrote posts to these groups', () => {
    let luna, mars, venus, privateGroup, protectedGroup, pubicGroup, anonResponse, authResponse;
    ...
    before(async () => {
      ...
      await Promise.all([
        ...
        testHelper.createAndReturnPostToFeed(
          protectedGroup,
          luna,
          'Post by Luna to Protected group',
        ),
        testHelper.createAndReturnPostToFeed(privateGroup, luna, 'Post by Luna to Private group'),

        testHelper.createAndReturnPostToFeed(
          [pubicGroup, protectedGroup],
          mars,
          'Post by Mars to Public and Protected groups',
        ),

        testHelper.createAndReturnPostToFeed(
          protectedGroup,
          venus,
          'Post by Venus to Protected group',
        ),
        testHelper.createAndReturnPostToFeed(privateGroup, venus, 'Post by Venus to Private group'),
      ]);

      // Fetch allGroups info
      [anonResponse, authResponse] = await Promise.all([
        getAllGroups(app),
    ...
    describe('Anonymous request to /v2/allGroups', () => {
      ...
      it('should return a proper structure', () =>
        expect(response, 'to exhaustively satisfy', allGroupsResponse));
      it('should return only one group', () => expect(response.groups, 'to have length', 1));
      it('it should be Public group', () =>
        expect(response.groups[0].id, 'to equal', pubicGroup.id));
      it('should be 3 subscribers in Public group', () =>
        expect(response.groups[0].subscribers, 'to equal', 3));
      it('should be about 2 posts in Public group', () =>
        expect(response.groups[0].postsByMonth, 'to be close to', 2, 1e-5));
    ...
    describe('Authorized request to /v2/allGroups', () => {
      ...
      it('should be 3 subscribers in Public group', () =>
        expect(pubicGroupResp.subscribers, 'to equal', 3));
      it('should be 3 subscribers in Protected group', () =>
        expect(protectedGroupResp.subscribers, 'to equal', 3));
      it('should be about 2 posts in Public group', () =>
        expect(pubicGroupResp.postsByMonth, 'to be close to', 2, 1e-5));
      it('should be about 3 posts in Protected group', () =>
        expect(protectedGroupResp.postsByMonth, 'to be close to', 3, 1e-5));
      it('Public group should have author variety about 50%', () => {
...
async function getAllGroups(app, user = null) {
  ...
  return await fetch(`${app.context.config.host}/v2/allGroups`, { headers }).then((r) => r.json());
}
