...
const searchTrait = (superClass) =>
  class extends superClass {
    async searchPosts(
      ...
      const bannedUsersFilter = this._getPostsFromBannedUsersSearchFilterCondition(
        bannedUserIds,
        feedIntIdsBannedForUser,
      );
      const bannedCommentAuthorFilter = this._getCommentsFromBannedUsersSearchFilterCondition(
        bannedUserIds,
      );
      ...
      const publicOrVisibleForAnonymous = currentUserId
        ? 'not users.is_private'
        : 'not users.is_protected';

      if (!visibleFeedIds || visibleFeedIds.length == 0) {
        visibleFeedIds = 'NULL';
      }

      const publicPostsSubQuery =
      ...
      const publicPostsByCommentsSubQuery =
        'select "posts".* from "posts" ' +
        'inner join "feeds" on posts.destination_feed_ids # feeds.id > 0 and feeds.name=\'Posts\' ' +
        `inner join "users" on feeds.user_id=users.uid and ${publicOrVisibleForAnonymous} ` +
        `where
      ...
      if (currentUserId) {
        ...
        const visiblePrivatePostsSubQuery =
          'select "posts".* from "posts" ' +
          'inner join "feeds" on posts.destination_feed_ids # feeds.id > 0 and feeds.name=\'Posts\' ' +
        ...
        const visiblePrivatePostsByCommentsSubQuery =
          'select "posts".* from "posts" ' +
          'inner join "feeds" on posts.destination_feed_ids # feeds.id > 0 and feeds.name=\'Posts\' ' +
          'inner join "users" on feeds.user_id=users.uid and users.is_private=true ' +
          `where
        ...
        subQueries = [
          ...subQueries,
          myPostsSubQuery,
          myPostsByCommentsSubQuery,
          visiblePrivatePostsSubQuery,
          visiblePrivatePostsByCommentsSubQuery,
        ];
      }
      ...
      return await this.database.transaction(async (trx) => {
        ...
        const { rows } = await trx.raw(
          `select uid from (${subQueries.join(
            ' union ',
          )}) as found_posts order by found_posts.bumped_at desc offset ${offset} limit ${limit}`,
        );
    ...
    async searchUserPosts(
      ...
      return await this.database.transaction(async (trx) => {
        ...
        const { rows } = await trx.raw(
          `select uid from (${subQueries.join(
            ' union ',
          )}) as found_posts where found_posts.user_id='${targetUserId}' order by found_posts.bumped_at desc offset ${offset} limit ${limit}`,
        );
        return rows.map((r) => r.uid);
      });
    }

    async searchGroupPosts(
      query,
      groupFeedId,
      authorId,
      visibleFeedIds,
      ...
      return await this.database.transaction(async (trx) => {
        ...
        const { rows } = await trx.raw(
          `select uid from (${subQueries.join(
            ' union ',
          )}) as found_posts ${authorCondition} order by found_posts.bumped_at desc offset ${offset} limit ${limit}`,
        );
        return rows.map((r) => r.uid);
      });
    }
    ...
    _getTextSearchCondition(parsedQuery, textSearchConfigName) {
      ...
      if (parsedQuery.query.length > 2) {
        const sql = pgFormat(
          `to_tsvector(%L, posts.body) @@ to_tsquery(%L, %L)`,
    ...
    _getCommentSearchCondition(parsedQuery, textSearchConfigName) {
      ...
      if (parsedQuery.query.length > 2) {
        const sql = pgFormat(
          `to_tsvector(%L, comments.body) @@ to_tsquery(%L, %L)`,
          textSearchConfigName,
          textSearchConfigName,
          parsedQuery.query,
        );
