...
const subscriptionsTrait = (superClass) =>
  class extends superClass {
    ...
    async isUserSubscribedToOneOfTimelines(currentUserId, timelineIds) {
      const q = pgFormat(
        'SELECT COUNT(*) AS "cnt" FROM "subscriptions" WHERE "feed_id" IN (%L) AND "user_id" = ?',
        timelineIds,
      );
    ...
    async subscribeUserToUser(subscriberId, targetId) {
      ...
      const publicFeedIds = ['Posts', 'Comments', 'Likes'].map((n) => allFeeds[n]).filter(Boolean); // groups only have 'Posts' feeds
