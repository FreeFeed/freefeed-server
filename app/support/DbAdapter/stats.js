    ...
    async getArchivesStats() {
      ...
      const [
        ...
        this.database('posts')
          .count('id')
          .first()
          .where('created_at', '<', FREEFEED_START_DATE),
        this.database('comments')
          .count('id')
          .first()
          .where('created_at', '<', FREEFEED_START_DATE),
        this.database('hidden_comments')
          .count('comment_id')
          .first(),
        ...
        this.database('archives')
          .count('user_id')
          .first()
          .where('restore_comments_and_likes', true),
      ]);
