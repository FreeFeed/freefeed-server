...
const postsTrait = (superClass) =>
  class extends superClass {
    ...
    async getPostById(id, params) {
      ...
      const attrs = await this.database('posts')
        .first()
        .where('uid', id);
    ...
    async withdrawPostFromCommentsFeedIfNoMoreComments(postId, commentatorId) {
      await this.database.transaction(async (trx) => {
        ...
        if (rows.length === 0) {
          ...
          await trx.raw(`update posts set feed_ids = feed_ids - :intId::int where uid = :postId`, {
            intId,
            postId,
          });
        }
    ...
    async getTimelinePostsIds(timelineIntIds = null, viewerId = null, params = {}) {
      ...
      if (viewerId) {
        [visiblePrivateFeedIntIds, bannedUsersIds] = await Promise.all([
      ...
      if (timelineIntIds) {
        ...
        if (params.activityFeedIds.length > 0) {
          sourceConditionParts.push(
            pgFormat(
              'p.feed_ids && %L and p.is_propagable',
              `{${params.activityFeedIds.join(',')}}`,
            ),
          );
        }
      ...
      const privacyCondition = viewerId
        ...
        : 'not p.is_protected';
      const bansSQL =
        bannedUsersIds.length > 0 ? pgFormat(`(not p.user_id in (%L))`, bannedUsersIds) : 'true';
    ...
    async getTimelinesIntersectionPostIds(timelineId1, timelineId2) {
      const res1 = await this.database('posts')
        .select('uid', 'updated_at')
        .orderBy('bumped_at', 'desc')
        .whereRaw('feed_ids && ?', [[timelineId1]]);
      const postIds1 = res1.map((record) => {
        return record.uid;
      });

      const res2 = await this.database('posts')
        .select('uid', 'updated_at')
        .orderBy('bumped_at', 'desc')
        .whereRaw('feed_ids && ?', [[timelineId2]]);
    ...
    bestPostsIds = async (currentUser, offset = 0, limit = 30) => {
      ...
      const publicOrVisibleForAnonymous = currentUser
    ...
    async setOldPostName(postId, oldName, oldUrl) {
      return await this.database('archive_post_names').insert({
        post_id: postId,
        old_post_name: oldName,
        old_url: oldUrl,
      });
    }
