...
const appTokensTrait = (superClass) =>
  class extends superClass {
    async createAppToken(payload) {
      ...
      const [id] = await this.database('app_tokens')
        .returning('uid')
    ...
    async getAppTokenById(uid) {
      const row = await this.database('app_tokens')
        .first()
        .where({ uid });
    ...
    async getActiveAppTokenByIdAndIssue(uid, issue) {
      const row = await this.database('app_tokens')
        .first()
        .where({ uid, issue, is_active: true });
      return initAppTokenObject(row);
    }
    ...
    updateAppToken(id, payload) {
      ...
      return this.database('app_tokens')
        .where('uid', id)
    ...
    async listActiveAppTokens(userId) {
      const { rows } = await this.database.raw(
        `select * from app_tokens where user_id = :userId and is_active order by created_at desc`,
        { userId },
      );
