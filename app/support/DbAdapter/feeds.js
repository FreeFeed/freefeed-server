...
const feedsTrait = (superClass) =>
  class extends superClass {
    ...
    async getAllUserNamedFeed(userId, name) {
      ...
      order by ord asc nulls first, created_at desc`,
        { userId, name },
      );
    ...
    async addNamedFeed(userId, name, title) {
      const [row] = await this.database('feeds')
        .returning('*')
        .insert({
          title,
          name,
    ...
    async destroyFeed(feedId, params = {}) {
      ...
      if (thisFeed.name !== 'RiverOfNews') {
        ...
        return !!(await this.database.raw(`delete from feeds where uid = :feedId returning true`, {
          feedId,
        }));
      }
