...
const USER_COLUMNS_MAPPING = {
  ...
  isVisibleToAnonymous: (is_visible_to_anonymous) => {
    return is_visible_to_anonymous === '1';
  },
  isRestricted: (is_restricted) => {
...
export class PgAdapter {
  ...
  async createAttachment(payload) {
    const preparedPayload = this._prepareModelPayload(
      payload,
      ATTACHMENT_COLUMNS,
  ...
  async createPost(payload, destinations, usages) {
    ...
    const destTimelineIntIds = await this.getTimelinesIntIdsByUUIDs(destinations);
    preparedPayload.destination_feed_ids = destTimelineIntIds;
    const usagesTimelineIntIds = await this.getTimelinesIntIdsByUUIDs(usages);
  ...
  async subscribeUserToTimeline(timelineId, currentUserId, timestamp) {
    ...
    const [feedIntId] = await this.getTimelinesIntIdsByUUIDs([timelineId]);

    return this.database.raw(
      'UPDATE users SET subscribed_feed_ids = uniq(subscribed_feed_ids + ?) WHERE uid = ?',
      [[feedIntId], currentUserId],
    );
  }
}
